...
const CalendarComponent = props => {
  const [selectedDate, setSelectedDate] = useState(
    new Date(moment(props?.selectedDate?.date)),
  );
  ...
  const checkIsSlotAvailable = (date, month, year) => {
    const selectedDate = moment(new Date(year, month, date)).format(
      'YYYY-MM-DD',
    );
    const isSlotAvailable = _.find(props.service_dates, {
      date: selectedDate,
      isSlotAvailable: true,
    });
    return !_.isEmpty(isSlotAvailable);
  };

  const checkIsWeekOff = (date, month, year) => {
    const selectedDate = moment(new Date(year, month, date)).format(
      'YYYY-MM-DD',
    );
    const isWeekOff =
      props?.service_dates.filter(
        item =>
          item.date == selectedDate &&
          (item.isWeekOff == true || item.isHoliday == true),
      ).length > 0;
    return isWeekOff;
  };
  ...
  const renderHeader = () => {
    return (
      <View style={styles?.row_1}>
        ...
        <Text style={styles?.month}>{`${months[calendar.month]} ${
          calendar.year
        }`}</Text>
                  ...
                  return (
                    <TouchableWithoutFeedback
                      ...
                      onPress={() => {
                        if (
                          date?.currentMonth &&
                          checkIsSlotAvailable(date.date, date.month, date.year)
                        ) {
                        ...
                        style={[
                          ...
                          date.currentMonth
                            ? checkIsSlotAvailable(
                              ...
                              : checkIsWeekOff(date.date, date.month, date.year)
                              ? styles?.isHoliday
